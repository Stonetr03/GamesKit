-- Stonetr03 - GamesKit - Yahtzee Scorecard Turn Style
    ...
    if d.turn and d.players then
        ...
        if p then
            if p == game.Players.LocalPlayer then
                ...
                local act = Module.Active:get();
                if act == 1 then
                    if Module.View1:get() ~= p then
                        -- switch to 2
                        Module.View2:set(p);
                        Module.Active:set(2);
                    end
                elseif act == 2 then
                    if Module.View2:get() ~= p then
                        -- switch to 1
                        Module.View1:set(p);
                        Module.Active:set(1);
                    end
...
function Module.Ui(data: table): {GuiObject}
    local disconnect = Fusion.Observer(data.data):onChange(function()
        update(data)
    end);
    update(data);
    return {
        ...
        New "Frame" {
            ...
            [Children] = {
                ...
                Fusion.ForPairs(scorecardText,function(i,o)
                    local p = Module.View2:get();
                    if not p then return i,nil end;
                    if i == 1 then
                        return titleUi(i,Computed(function()
                            -- Text
                            return p.Name;
