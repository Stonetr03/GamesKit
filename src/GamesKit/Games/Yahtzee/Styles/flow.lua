-- Stonetr03 - GamesKit - Yahtzee Flow Style
    -- Each player goes at their own pace
...
function Module.CreateInitData(g: table)
    g.data = {
        style = "flow";
        players = {};
        done = 0; -- amount of players done with game
    }
    for _,p in pairs(g.players) do
        g.data.players[p.UserId] = {
            score = table.create(13,-1); -- -1 indicated not scored.
            dice = table.create(5,0); -- 0 indicates not rolled.
...
function Module.Roll(g: table, p: Player, f1: boolean, f2: boolean, f3: boolean, f4: boolean, f5: boolean): boolean
    if g.data.players[p.UserId] then
        ...
        if canRoll and g.data.players[p.UserId].roll < 3 then
...
function Module.Score(g: table, p: Player, i: number, dblIndex: number?): boolean
    ...
    if g.data.players[p.UserId] then
        ...
        if canRoll and g.data.players[p.UserId].roll > 0 and (g.data.players[p.UserId].score[i] == -1 or (i == 12 and getScore(i,g.data.players[p.UserId].dice,g.data.players[p.UserId].score[12]) > g.data.players[p.UserId].score[12])) then
